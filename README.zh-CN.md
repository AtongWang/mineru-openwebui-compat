...
**为尚未适配 MinerU 4 API 的 Open WebUI 提供 `/file_parse` 中转服务。**

Open WebUI v0.11.1 的本地 MinerU 集成仍向旧的 `POST /file_parse` 接口上传文件，并同步读取返回的 Markdown。MinerU 4 已移除该接口，改为 V1 的“上传文件 → 提交解析任务 → 轮询 → 下载结果”流程，因此不能直接连接，单纯修改 URL 也无法解决。

本项目就是为填补这段接口差异而构建：对 Open WebUI 提供它认识的 `/file_parse`，向已有 MinerU 4 服务调用 V1 API，再将 Markdown 包装为旧的 `results.<文件名>.md_content` 响应。

```text
Open WebUI
    │ POST /file_parse（同步文件上传）
    ▼
兼容中转服务（仅 CPU，无模型）
    │ V1 上传 → 解析任务 → 轮询 → 下载 Markdown
    ▼
已有 MinerU 4 API → 推理模型 / GPU
```

无需修改 Open WebUI 源码，也不需要额外加载模型。**需已有可用的 MinerU 4 API 服务**；本仓库不安装或启动推理模型，也不是完整的 MinerU 3 API 实现。兼容性结论针对已检查的 Open WebUI v0.11.1；未来版本若原生支持 MinerU 4 V1 API，可直接连接，不再需要此中转层。
